...
static SignalFunction default_signal_functions[ARRAY_SIZE(exception_signals)];
...
static void fail_if_leftover_values(const char* test_name)
{
    ...
    if (check_for_leftover_values_list(
...
static void teardown_testing(const char* test_name)
{
    ...
    initialize_source_location(&global_last_parameter_location);
    list_free(&global_call_ordering_head, free_value, (void*)0);
...
static void free_symbol_map_value(const void* value, void* cleanup_value_data)
{
    ...
    assert_non_null(value);
    list_free(&map_value->symbol_values_list_head, children ? free_symbol_map_value : free_value,
                             ...
                             const int refcount)
{
    ...
    {
        add_symbol_value(&target_map_value->symbol_values_list_head, &symbol_names[1],
                         number_of_symbol_names - 1, value, refcount);
    }
}
 ...
 */
static int get_symbol_value(ListNode* const head,
                            const char* const symbol_names[],
                            const size_t number_of_symbol_names,
                            void** output)
{
    ...
    if (list_find(head, symbol_name, symbol_names_match, &target_node))
    {
        ...
        {
            return_value = get_symbol_value(child_list, &symbol_names[1],
...
static void remove_always_return_values_from_list(ListNode* const map_head)
{
    ...
    for (current = map_head->next, next = current->next; current != map_head;
...
LargestIntegralType _mock(const char* const function, const char* const file, const int line)
{
    void* result;
    const int rc =
        get_symbol_value(&global_function_result_map_head, &function, 1, &result);
    if (rc)
    {
    ...
    if (rc)
    {
       ...
       */
            if (cmp != 0)
            {
                ...
                cmp = strcmp(expected_call->function, function);
                while (value_node->refcount < -1 && cmp != 0 &&
        ...
        else
        {
            cm_print_error(SOURCE_LOCATION_FORMAT
                           ...
                           "in %s\n",
                           file, line, expected_call->function, function);
            exit_test(1);
        }
    }
    else
...
void _will_return(const char* const function_name,
                  ...
                  const int count)
{
    SymbolValue* const return_value = (SymbolValue*)malloc(sizeof(*return_value));
    assert_true(count != 0);
...
{
    return value_in_set_display_error(
        value, cast_largest_integral_type_to_pointer(CheckIntegerSet*, check_value_data), 0);
}
...
{
    return value_in_set_display_error(
        value, cast_largest_integral_type_to_pointer(CheckIntegerSet*, check_value_data), 1);
}
...
static void expect_set(const char* const function,
                       ...
                       const int count)
{
    CheckIntegerSet* const check_integer_set = (CheckIntegerSet*)malloc(
        sizeof(*check_integer_set) + (sizeof(values[0]) * number_of_values));
...
static int check_in_range(const LargestIntegralType value, const LargestIntegralType check_value_data)
{
    CheckIntegerRange* const check_integer_range =
        cast_largest_integral_type_to_pointer(CheckIntegerRange*, check_value_data);
...
void _expect_string(const char* const function,
                    ...
                    const int count)
{
    declare_initialize_value_pointer_pointer(string_pointer, discard_const(string));
...
static int check_memory(const LargestIntegralType value, const LargestIntegralType check_value_data)
{
    CheckMemoryData* const check =
        cast_largest_integral_type_to_pointer(CheckMemoryData*, check_value_data);
                     ...
                     const LargestIntegralType value)
{
    ...
    const int rc =
        get_symbol_value(&global_function_parameter_map_head, symbols, 2, &result);
    if (rc)
    {
                         ...
                         const int line)
{
    ...
    if (result > valmax - 1)
    {
        ...
        {
            cm_print_error(
                "%s < 0, errno(" LargestIntegralTypePrintfFormatDecimal
                "): %s\n",
...
void* _test_malloc(const size_t size, const char* file, const int line)
{
    ...
    const size_t allocate_size =
        size + (MALLOC_GUARD_SIZE * 2) + sizeof(*block_info) + MALLOC_ALIGNMENT;
    char* const block = (char*)malloc(allocate_size);
    assert_non_null(block);

    /* Calculate the returned address. */
    ptr = (char*)(((size_t)block + MALLOC_GUARD_SIZE + sizeof(*block_info) + MALLOC_ALIGNMENT) &
                  ~(MALLOC_ALIGNMENT - 1));
            ...
            for (j = 0; j < MALLOC_GUARD_SIZE; j++)
            {
                ...
                if (diff)
                {
                    cm_print_error(SOURCE_LOCATION_FORMAT
                                   ...
                                   file, line, ptr, (unsigned long)block_info->size,
                                   block_info->location.file,
                                   block_info->location.line, (void*)&guard[j]);
                    _fail(file, line);
                }
            }
        }
...
static int display_allocated_blocks(const ListNode* const check_point)
{
    ...
    for (node = check_point->next; node != head; node = node->next)
    {
        ...
        cm_print_error(
            SOURCE_LOCATION_FORMAT ": note: block %p allocated here\n",
...
void _fail(const char* const file, const int line)
{
    ...
    case CM_OUTPUT_STDOUT:
        cm_print_error(
            "[   LINE   ] --- " SOURCE_LOCATION_FORMAT ": error: Failure!", file, line);
        break;
    default:
...
static void exception_handler(int sig)
{
    ...
    cm_print_error("Test failed with exception: %s(%d)", sig_strerror, sig);
    exit_test(1);
}
                                      ...
                                      struct CMUnitTestState* cm_tests)
{
    ...
    fprintf(fp,
            "  <testsuite name=\"%s\" time=\"%.3f\" "
            "tests=\"%u\" failures=\"%u\" errors=\"%u\" skipped=\"%u\" >\n",
            group_name, total_runtime * 1000, /* miliseconds */
            (unsigned)total_executed, (unsigned)total_failed,
            (unsigned)total_errors, (unsigned)total_skipped);
                                  ...
                                  struct CMUnitTestState* cm_tests)
{
    ...
    {
    case CM_OUTPUT_STDOUT:
        cmprintf_group_finish_standard(total_executed, total_passed, total_failed,
                                       total_errors, total_skipped, cm_tests);
        break;
    case CM_OUTPUT_SUBUNIT:
        break;
    case CM_OUTPUT_TAP:
        ...
        break;
    case CM_OUTPUT_XML:
        cmprintf_group_finish_xml(group_name, total_executed, total_failed, total_errors,
                                  total_skipped, total_runtime, cm_tests);
        break;
    }
}
...
static struct timespec cm_tspecdiff(struct timespec time1, struct timespec time0)
{
    struct timespec ret;
    int xsec = 0;
    int sign = 1;
...
{
    const ListNode* const volatile check_point =
        (const ListNode*)(heap_check_point != NULL ? heap_check_point
                                    ...
                                    const void* const heap_check_point)
{
    ...
    if (setup_func != NULL)
    {
        rc = cmocka_run_one_test_or_fixture(function_name, NULL, setup_func,
                                            NULL, state, heap_check_point);
    }
    else
    {
        rc = cmocka_run_one_test_or_fixture(
            function_name, NULL, NULL, teardown_func, state, heap_check_point);
    }
...
static int cmocka_run_one_tests(struct CMUnitTestState* test_state)
{
    ...
    if (test_state->test->setup_func != NULL)
    {
        ...
        rc = cmocka_run_one_test_or_fixture(
            ...
            &test_state->state, test_state->check_point);
        if (rc != 0)
        {
    ...
    if (rc == 0)
    {
        rc = cmocka_run_one_test_or_fixture(test_state->test->name,
                                            test_state->test->test_func, NULL,
                                            NULL, &test_state->state, NULL);
        if (rc == 0)
        {
    ...
    {
        rc = cmocka_run_one_test_or_fixture(
            test_state->test->name, NULL, NULL, test_state->test->teardown_func,
            &test_state->state, test_state->check_point);
        if (rc != 0)
        {
            test_state->status = CM_TEST_ERROR;
                            ...
                            CMFixtureFunction group_teardown)
{
    ...
    {
        if (tests[i].name != NULL && (tests[i].test_func != NULL || tests[i].setup_func != NULL ||
    ...
    if (rc == 0)
    {
        ...
        for (i = 0; i < total_tests; i++)
        {
            ...
            if (rc == 0)
            {
                ...
                {
                case CM_TEST_PASSED:
                    cmprintf(PRINTF_TEST_SUCCESS, test_number,
                             cmtest->test->name, cmtest->error_message);
                    total_passed++;
                    break;
                case CM_TEST_SKIPPED:
                    cmprintf(PRINTF_TEST_SKIPPED, test_number,
                             cmtest->test->name, cmtest->error_message);
                    total_skipped++;
                    break;
                case CM_TEST_FAILED:
                    cmprintf(PRINTF_TEST_FAILURE, test_number,
                             cmtest->test->name, cmtest->error_message);
                    total_failed++;
                    break;
                default:
                    cmprintf(PRINTF_TEST_ERROR, test_number, cmtest->test->name,
    ...
    else
    {
        ...
        cmprintf(PRINTF_TEST_ERROR, 0, group_name, "[  FAILED  ] GROUP SETUP");
        total_errors++;
    }
    ...
    {
        rc = cmocka_run_group_fixture("cmocka_group_teardown", NULL, group_teardown,
                                      &group_state, group_check_point);
        if (rc != 0)
        {
            if (cm_error_message != NULL)
            ...
            cmprintf(PRINTF_TEST_ERROR, 0, group_name,
                     "[  FAILED  ] GROUP TEARDOWN");
        }
    }

    cmprintf_group_finish(group_name, total_executed, total_passed, total_failed,
                          total_errors, total_skipped, total_runtime, cm_tests);
...
{
    const ListNode* const volatile check_point =
        (const ListNode*)(heap_check_point ? heap_check_point
                                           : check_point_allocated_blocks());
    void* current_state = NULL;
    volatile int rc = 1;
    int handle_exceptions = 1;
...
int _run_tests(const UnitTest* const tests, const size_t number_of_tests)
{
   ...
   */
    TestState* test_states = (TestState*)malloc(number_of_tests * sizeof(*test_states));
    /* The number of test states which should be 0 at the end */
    long number_of_test_states = 0;
    /* Names of the tests that failed. */
    const char** failed_names =
        (const char**)malloc(number_of_tests * sizeof(*failed_names));
    void** current_state = NULL;
    ...
    {
        print_error("[  ERROR   ] Mismatched number of setup %" PRIdS
                    " and "
...
int _run_group_tests(const UnitTest* const tests, const size_t number_of_tests)
{
    ...
    for (i = 0; i < number_of_tests; i++)
    {
        ...
        if (test->function_type == UNIT_TEST_FUNCTION_TYPE_GROUP_SETUP)
        {
            ...
            {
                print_error(
                    "[  ERROR   ] More than one group setup function "
                    "detected\n");
                exit_test(1);
            }
        }
        ...
        if (test->function_type == UNIT_TEST_FUNCTION_TYPE_GROUP_TEARDOWN)
        {
            ...
            {
                print_error(
                    "[  ERROR   ] More than one group teardown function "
                    "detected\n");
                exit_test(1);
            }
        }
    ...
    if (setup != NULL)
    {
        ...
        *current_state = NULL;
        failed = _run_test(setup_name, setup, current_state,
                           UNIT_TEST_FUNCTION_TYPE_SETUP, group_state.check_point);
        if (failed)
        {
    ...
    while (current_test < number_of_tests)
    {
        ...
        if (run_test)
        {
            ...
            failed = _run_test(test->name, test->function, current_state,
                               test->function_type, NULL);
            if (failed)
            {
                failed_names[total_failed] = test->name;
            }
    ...
    if (teardown != NULL)
    {
        ...
        failed = _run_test(teardown_name, teardown, current_state,
                           UNIT_TEST_FUNCTION_TYPE_GROUP_TEARDOWN, group_state.check_point);
        if (failed)
        {
